...
date: 2026-05-28T19:03:17+08:00
draft: false
tags: ["英语"]
featured: false
mood: "focus"
...
离开前请确保锁好门
主将从现：当主句为将来时，状语从句用现在时表将来。
时间状语从句：as soon as+时间状语，when+时间状语，after，before，until, once
She will call me when she gets home. 她到家后会给我打电话的。
...
I will help you if you need me。如果你需要我，我会帮助你的
让步状语从句：whatever, whoever, wherever, whether...or not
Whatever happens, I will support you. 无论发生什么事，我都会支持你。
...
### 就远原则
1. as well as
2. together/along/combined with
3. with
4. rather than
5. except
6. besides
...
这些动词包括：agree, aim, arrange, afford, ask, beg, choose, claim, decide, demand, desire, hesitate, dare, determine, expect, fail, hope, help, learn, manage, offer, plan, prepare, pretend, promise, refuse, volunteer, want, wish, would like, would love 等等。这些动词后面必须用“to do”形式，不能用其他形式，比如动名词或分词。
...
advise, suggest, discuss, practice, allow, permit, appreciate, enjoy, finish, burst out, can’t resist, can’t stand, can’t help, consider, deny, avoid, miss, escape, forbid, imagine, mind, delay, postpone, risk, bear, tolerate, ban 等。
有些以介词结尾的动词词组也只能用 doing 形式作宾语，如：
admit to, be accustomed to, be used to, contribute to, devote to, confess to, get down to, look forward to, stick to, object to, pay attention to, when it comes to, submit to, accuse sb. of, aim at, apologize to sb. for, be fond of, be responsible for, believe in, depend on, dream of, feel like, give up, keep on, insist on, persist in, prevent from, put off, succeed in, think of, thank sb. for, worry about, be busy (in), have difficulty/trouble (in), have a good/wonderful/hard time (in) 等。
...
2. ___(live) in Beljing for 10 years,Tom can now speak fluent Mandarin
- Tome 主动 live(住)
- live 住这个动作很多年前,先于谓语
- having lived
...
2. 表示与事实相反的虚拟语气, 意为"假如"、"假设", that 可以省略。
...
- 2. if only 要是/ would rather
- - 过去：had done
- - 现在/将来：did
- were
- 3. but for / but that / otherwise / without 多用 would
...
interesting 来说，因为它有三个音节，所以不能直接加 “-er”，而是需要用 “more” 来构成比较级，构成 “more interesting”，“much” 是一个加强语气的副词，用来修饰比较级，但不能单独构成比较级。比较级结构：“more + 形容词 + than + 比较对象”。
例题 3：
...
1. must have + 过去分词：（过去）一定做了……。
He must have left his phone at home.（他一定把手机落在家里了。）
2. should/ought to have + 过去分词：（过去）本应该做……（但没做）。
You should have told me earlier.（你本该早点告诉我的。）
3. can't have done: (过去)不可能做了……，对过去情况的否定推测。
4. shouldn't/oughtn't to have done:(过去)本不该做……(却做了)，后悔之意。
5. needn't have done:(过去)本不必做……(却做了)。
...
1. If you don’t like to swim, you ____ as well stay at home.
...
A. / B. the C. an D. a
以 -ics 结尾表示学科等的名词作主语时，一般要求谓语动词用单数形式：mathematics (数学)、physics (物理学)、economics (经济学)，electronics (电子学)、ethics (伦理学)、politics (政治学)、statistics (统计学)、gymnastics (体操)、athletics (田径运动) 等。economics、electronics、ethics、politics、statistics 表示学科以外的其他含义时，可作 复数 用。
...
- 1. 修饰单数可数名词：a/an 在形容词前用 such，a/an 在形容词后用 so:
...
- such + 形容词 + 不可数名词
- such great progress
- 2. so + many/much/few/little + 可数名词复数/不可数名词
- so many students
- so much food
...
4. So…that…这里 so 后面应该接 adj.但是 much many few little 这些词修饰后，也可以接名词
...
## 主动形式表被动意义
1. 以下动词等后面接动名词时，该动名词通常用主动形式表示被动意义。
bear(适宜于)
deserve(应受到)
need(需要)
require(需要)
...
1. need 主动表示被动: 需要做某事, 种动作是需要被执行.
This room needs cleaning.（这个房间需要清洁。）
This car needs washing.（这辆车需要洗。）
2. be needed 被动语态: 某人或某物**被需要,某个对象被需要。
